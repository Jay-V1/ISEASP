              ...
              <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped">
                  <tbody>
                    ...
                    <?php 
                        $sql = "SELECT * FROM `tblemployers` c,`tbljobregistration` j, `tblfeedback` f, `tbljob` jb  WHERE jb.`COMPANYID`=c.`EMPLOYERID` AND c.`EMPLOYERID`=j.`COMPANYID` AND j.`REGISTRATIONID`=f.`REGISTRATIONID` AND `PENDINGAPPLICATION`=0 AND j.`APPLICANTID`='{$_SESSION['APPLICANTID']}'";
                        $mydb->setQuery($sql);
                        ...
                        foreach ($cur as $result) {
                          ...
                          echo '<td class="mailbox-position">'.$result->OCCUPATIONTITLE.'</td>';
                          echo '<td class="mailbox-subject">'.$result->REMARKS.'</td>';
                          echo '<td class="mailbox-date">'.$result->DATETIMEAPPROVED.'</td>';
                          echo '<td><a href="index.php?view=replymessage&id='.$result->REGISTRATIONID.'" class="btn btn-xs btn-primary"><i class="fa fa-reply"></i> Reply</a></td>';
